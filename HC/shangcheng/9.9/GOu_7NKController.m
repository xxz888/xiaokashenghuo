...
@interface GOu_7NKController ()<RPTaggedNavViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation GOu_7NKController
...
- (void)spyFilthyObedienceCleave{
    ...
    [self NetWorkingPostWithURL:self hiddenHUD:NO url:@"/api/shop/api/getGood/99" Params:params success:^(id  _Nonnull responseObject) {
        ...
        if ([responseObject[@"status"] intValue] == 200) {
            ...
            if (self.tagged > 0) {
                [self insubordinateSalt:self.tagged];
            }else{
...
- (void)viewDidLoad {
    ...
    self.tagged = 0;
    [self psycheMalediction];
    [self spyFilthyObedienceCleave];
}
...
- (UICollectionView *)collectionview{
    if (!_collectionview) {
        ...
        _collectionview.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
            ...
            weak_self.page = 1;
            [weak_self spyFilthyObedienceCleave];
...
- (void)insubordinateSalt:(NSInteger)index{
    if (index == 0) {
