...
@interface WDtWubmf_r3Controller ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation WDtWubmf_r3Controller
...
- (void)everyoneIncredulityPredict {
...
- (UIButton *)tishiBtn{
    if (!_tishiBtn) {
        ...
        [_tishiBtn setImage:[UIImage imageNamed:@"altogetherLaboredDesiccate"] forState:UIControlStateNormal];
...
- (void)swarmContemporary{
    DHSDpnz_x3 *model = self.dataArr[0];
...
- (void)passiveObsolescentTulip{
    ...
    [self NetWorkingPostWithURL:self hiddenHUD:NO url:@"/api/user/update/pwd" Params:dic success:^(id  _Nonnull responseObject) {
        if ([responseObject[@"code"] intValue] == 0) {
            ...
            [ZIONf_d showBottomWithText:@"密码修改成功,请重新登录"];
