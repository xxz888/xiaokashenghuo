...
@interface EQDbtKp_WVkController ()
...
@implementation EQDbtKp_WVkController
...
- (UIImageView *)bg_image{
    if (!_bg_image) {
        _bg_image = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"precipitantPoet"]];
...
-(CGFloat)satisfactoryConstituent{
    
    UIImage *img = [UIImage imageNamed:@"precipitantPoet"];
...
- (void)unexpectedYouthful{
    ...
    [self.bg_image mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.top.offset(0);
        make.height.offset([self satisfactoryConstituent]);
    }];
    ...
    _scrollView.contentSize = CGSizeMake(DEVICE_WIDTH, [self satisfactoryConstituent]+80);
}
